...
Page({
  ...
  init() {
    app.wxRequest(
      ...
      res => {
        if (res.data.code == 1) {
          res.data.data.logoImgUrl = tools.globalData.url + res.data.data.logoImgUrl
          this.setData({
  ...
  phoneTap() {
    wx.showModal({
      content: this.data.aboutMsg.serviceTel,
      confirmText: "拨打",
      confirmColor: "#5ac2bb",
      success: res => {
        if (res.confirm) {
          wx.makePhoneCall({
            phoneNumber: this.data.aboutMsg.serviceTel //仅为示例，并非真实的电话号码
          })
        }
      }
    })
