...
App({
  ...
  /*
   ...
   flag = -4 最多只能上传10张
   */
  uploadFile: function (name, cb, data, count, _url = '/api/commons/upload',params) {//name自定标识,cb回调函数,data其它表单json,params chooseImage参数{count:1}
    var that = this;
    var chooseParams = {
      count: count,
  ...
  //请求接口
  wxRequest: function (url, data, successCB, isNeedToken=true, failCB) {
    var that = this;
    ...
    wx.request({
      ...
      header: {
        ...
        'token': isNeedToken ? tools.getStorage('token') : tools.getStorage('operationToken')
      },
      data: data,
